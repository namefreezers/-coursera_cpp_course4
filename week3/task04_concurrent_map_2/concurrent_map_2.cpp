...
private:
    ...
    Hash hasher;
    size_t bucket_count;
    vector<unordered_map<K, V, Hash>> sub_maps;
    mutable vector<mutex> mutexes;
};

template<typename K, typename V, typename Hash>
ConcurrentMap<K, V, Hash>::ConcurrentMap(size_t bucket_count) : bucket_count(bucket_count),
                                                          sub_maps(vector<unordered_map<K, V, Hash>>(bucket_count)),
...
typename ConcurrentMap<K, V, Hash>::ReadAccess ConcurrentMap<K, V, Hash>::At(const K &key) const {
    auto ind = GetIndex(key);
    return {lock_guard<mutex>(mutexes[ind]), sub_maps[ind].at(key)};
}
...
bool ConcurrentMap<K, V, Hash>::Has(const K &key) const {
    auto ind = GetIndex(key);
    {
        lock_guard<mutex> lg(mutexes[ind]);
        return sub_maps[ind].find(key) != sub_maps[ind].end();
    }
}
