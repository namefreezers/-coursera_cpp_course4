...
public:
    ...
    class Assigner {
    private:
        std::vector<T> &inner_coeffs_;
        size_t degree_;

        T GetConstValue() const {
            return degree_ < inner_coeffs_.size() ? inner_coeffs_[degree_] : 0;
        }

    public:

        Assigner(std::vector<T> &inner_coeffs, std::size_t degree) : inner_coeffs_(inner_coeffs), degree_(degree) {}

        Assigner& operator=(T new_value) {
            if (new_value == 0  && degree_ >= inner_coeffs_.size()) {
                return *this;
            }

            if (degree_ >= inner_coeffs_.size()) {
                inner_coeffs_.resize(degree_ + 1);
            }
            inner_coeffs_[degree_] = new_value;

            // shrink
            while (inner_coeffs_.size() > 1 && inner_coeffs_.back() == 0) {
                inner_coeffs_.pop_back();
            }
            return *this;
        }

//        bool operator==(const T& another) const {
//            return GetConstValue() == another;
//        }
//
//        ostream &operator<<(ostream &os) const {
//            return os << GetConstValue();
//        }
    ...
    Assigner operator[](size_t degree) {
        return Assigner(coeffs_, degree);
    }
