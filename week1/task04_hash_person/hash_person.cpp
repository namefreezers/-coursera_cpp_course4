...
struct AddressHasher {
    size_t operator()(const Address& address) const {
        size_t x = 541;
        return string_hash(address.city)*x*x +string_hash(address.street)*x + address.building;
    }
...
struct PersonHasher {
    size_t operator()(const Person& person) const {
        size_t x = 541;
        return person.height*x*x*x+string_hash(person.name)*x*x +double_hash(person.weight)*x + address_hash(person.address);
    }
