    ...
    void CalculateBusLengthes() {
        for (auto&[name, bus] : buses) {
            ...
            while (next_it != bus.stops.end()) {
                res_length += stops.at(*prev_it).coords - stops.at(*next_it).coords;
                prev_it++, next_it++;
            }
...
struct GetBusRequest : public ReadRequest {
    ...
    string ServeRequest(const Database& db) {
        ...
        if (!bus_info) {
            ...
            ss << "Bus " << bus_name << ": " << bus_info->num_stops << " stops on route, " << bus_info->num_unique_stops << " unique stops, " <<  setprecision(6) << bus_info->bus_length << " route length";
        }
        return ss.str();
    }
};
